...
class FreqaiMultiFactorBtcV5Strategy(IStrategy):
    """
    WFA-1Y-ML-005 — The stop-loss test.
    IDENTICAL to ML-001 (FreqaiMultiFactorBtcStrategy) except ONE change:
    stoploss -5% → -1.5%.

    Hypothesis (from ML-001 decomposition): the model's quick-pop edge is
    captured fine by ROI (24 winners avg +1.67%); the entire -25.41% came
    from 13 stop-loss losers averaging -5.19%. Cutting losers at -1.5%
    should flip the run positive without touching the model or exits.
    """
    INTERFACE_VERSION = 3
    timeframe = "1h"
