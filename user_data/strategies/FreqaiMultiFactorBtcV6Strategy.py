...
class FreqaiMultiFactorBtcV6Strategy(IStrategy):
    """
    WFA-1Y-ML-006a — Target horizon test: 12h.
    IDENTICAL to ML-001 EXCEPT set_freqai_targets uses shift(-12) instead of
    shift(-3). Same features, same model config, same wrapper (ROI + -5% stop).

    Hypothesis: 3h raw BTC return is pure noise (calibration corr -0.017,
    macro features near-zero importance). 12h is trend-scale — should show
    positive prediction→actual correlation if the signal is learnable.
    """
    INTERFACE_VERSION = 3
    timeframe = "1h"
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        dataframe['&s-up_or_down'] = dataframe['close'].shift(-12) / dataframe['close'] - 1
        return dataframe
