...
class FreqaiMultiFactorBtcV7Strategy(IStrategy):
    """
    WFA-1Y-ML-006b — Target horizon test: 24h.
    IDENTICAL to ML-001 EXCEPT set_freqai_targets uses shift(-24) instead of
    shift(-3). Same features, same model config, same wrapper (ROI + -5% stop).

    Hypothesis: 24h BTC return is trend-scale — strongest candidate for a
    learnable target. Paired with V6 (12h) to find the horizon where the
    model's predictions actually correlate with realized returns.
    """
    INTERFACE_VERSION = 3
    timeframe = "1h"
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        dataframe['&s-up_or_down'] = dataframe['close'].shift(-24) / dataframe['close'] - 1
        return dataframe
