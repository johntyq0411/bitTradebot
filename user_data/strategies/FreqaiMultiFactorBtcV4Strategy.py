...
class FreqaiMultiFactorBtcV4Strategy(IStrategy):
    """
    WFA-1Y-ML-004 — Fixes 1+2 only (no SMA200 gate).
    Model/features IDENTICAL to ML-001. Wrapper changes:

    FIX 1 (win/loss asymmetry): minimal_roi disabled ({}), replaced by
        ATR-based trailing stop via custom_stoploss (3x ATR below max_rate).
        Winners can run; stoploss at -3% as disaster backstop.
    FIX 2 (horizon mismatch): custom_exit forces exit after 6h (2x the 3h
        prediction horizon) if profit < 1% — the signal has expired.
    (ML-002's SMA200 bull filter removed — over-filtered: 39 trades → 3,
     killed the profitable bear-bounce trades.)
    """
    INTERFACE_VERSION = 3
    timeframe = "1h"
    ...
    def custom_stoploss(self, pair: str, trade, current_time: datetime, current_rate: float,
        ...
        Trail 3x ATR below the highest price seen since entry (trade.max_rate).
        Never looser than the hard -3% stoploss.
        """
        ...
        stop_price = trade.max_rate - 3 * atr
        if stop_price <= 0:
            return -0.05
        return max((stop_price / current_rate) - 1, -0.03)
    ...
    def custom_exit(self, pair: str, trade, current_time: datetime, current_rate: float,
        ...
        if hours_held >= 24 and current_profit < 0.0:
            return 'horizon_stop'
        return None
